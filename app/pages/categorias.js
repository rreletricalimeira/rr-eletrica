...
export function renderCategorias(container) {
  container.innerHTML = `
    <div class="page">
      ...
      <div class="linha-dupla">
        <input id="nova-categoria" placeholder="Nova categoria de produtos" />
  ...
  function renderLista() {
    ...
    `).join('') || '<li><em>Nenhuma categoria de produtos cadastrada</em></li>';

    listaEl.querySelectorAll('.btn-excluir-item').forEach((btn) => {
      btn.addEventListener('click', async () => {
        if (!confirm('Excluir esta categoria de produtos?')) return;
