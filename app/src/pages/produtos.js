...
import { produtosAbaixoDoEstoque, tabelaEstoqueBaixoHtml } from '../ui/estoque_alerta.js';
...
export function renderProdutos(container) {
  ...
  function renderAlertaEstoque() {
    ...
    container.querySelector('#alerta-estoque').innerHTML = abaixo.length ? `
      <div class="card alerta-estoque">
        <h3>⚠ Produtos abaixo do estoque mínimo (${abaixo.length})</h3>
        ${tabelaEstoqueBaixoHtml(abaixo)}
      </div>
    ` : '';
  }

  function renderForm(id = null) {
    montarFormProduto(formWrap, id, {
      onSalvo: () => { renderLista(); },
      onExcluido: () => { renderLista(); },
    });
  }
...
export function montarFormProduto(formWrap, id = null, opcoes = {}) {
  // opcoes: { onSalvo(idProduto), onCancelar(), onExcluido(), nota, estoqueInicial }
  const { onSalvo = () => {}, onCancelar = () => {}, onExcluido = () => {}, nota = '' } = opcoes;
  const p = id ? all('SELECT * FROM produtos WHERE id = ?', [id])[0] : {};
  if (!id && opcoes.estoqueInicial !== undefined) p.estoque_atual = opcoes.estoqueInicial;

  formWrap.innerHTML = `
    <div class="card form-card">
      <h3>${id ? 'Editar produto' : 'Novo produto'}</h3>
      ${nota ? `<p class="item-sub campo-full">${escapeHtml(nota)}</p>` : ''}
  ...
  formWrap.querySelector('#btn-salvar-produto').addEventListener('click', async () => {
    ...
    if (id) {
      ...
      produtoId = all('SELECT last_insert_rowid() as id')[0].id;
    }
